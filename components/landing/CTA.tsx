'use client';

export default function CTA() {
  return (
    <section className="py-24 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-black">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl shadow-2xl overflow-hidden">
          <div className="grid lg:grid-cols-2 gap-12 p-12 lg:p-16">
            {/* Left Column - Main CTA */}
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="inline-block px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full text-white text-sm font-medium">
                  Buscando Socios & Colaboradores
                </div>
                <h2 className="text-4xl sm:text-5xl font-bold text-white leading-tight">
                  Construyamos el Futuro de la Salud Personalizada
                </h2>
                <p className="text-xl text-blue-50 leading-relaxed">
                  Estoy desarrollando este MVP y busco socios estratégicos,
                  inversores y colaboradores que compartan la visión de usar IA
                  para mejorar la salud y esperanza de vida de las personas.
                </p>
              </div>

              {/* What I'm Looking For */}
              <div className="space-y-3">
                <p className="text-lg font-semibold text-white">
                  Perfiles que busco:
                </p>
                <ul className="space-y-2 text-blue-50">
                  <li className="flex items-start gap-3">
                    <span className="text-2xl">💼</span>
                    <span>Co-fundadores con experiencia en healthtech o fitness tech</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-2xl">🏥</span>
                    <span>Profesionales de la salud (nutricionistas, médicos deportivos)</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-2xl">💻</span>
                    <span>Desarrolladores interesados en IA aplicada a salud</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="text-2xl">💰</span>
                    <span>Inversores angel o VCs enfocados en wellness & AI</span>
                  </li>
                </ul>
              </div>
            </div>

            {/* Right Column - Contact Options */}
            <div className="space-y-6">
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 space-y-6">
                <h3 className="text-2xl font-bold text-white mb-6">
                  Conectemos
                </h3>

                {/* LinkedIn Button */}
                <a
                  href="https://www.linkedin.com/in/tu-perfil"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-3 w-full px-6 py-4 bg-[#0077B5] hover:bg-[#006399] text-white font-semibold rounded-xl transition-all duration-300 hover:scale-105 shadow-lg"
                >
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                  </svg>
                  Conectar en LinkedIn
                </a>

                {/* Email Button */}
                <a
                  href="mailto:hernandez.hs@gmail.com?subject=Interés en App de Entrenamiento con IA"
                  className="flex items-center justify-center gap-3 w-full px-6 py-4 bg-white hover:bg-gray-100 text-gray-900 font-semibold rounded-xl transition-all duration-300 hover:scale-105 shadow-lg"
                >
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                  </svg>
                  Enviar Email
                </a>

                {/* Twitter/X Button */}
                <a
                  href="https://twitter.com/tu-usuario"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-3 w-full px-6 py-4 bg-black hover:bg-gray-900 text-white font-semibold rounded-xl transition-all duration-300 hover:scale-105 shadow-lg"
                >
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                  </svg>
                  Seguir en X/Twitter
                </a>
              </div>

              {/* Stats */}
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold text-white">MVP</div>
                  <div className="text-sm text-blue-100">En desarrollo</div>
                </div>
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 text-center">
                  <div className="text-3xl font-bold text-white">2026</div>
                  <div className="text-sm text-blue-100">Lanzamiento previsto</div>
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Additional Info */}
        <div className="mt-12 text-center">
          <p className="text-gray-600 dark:text-gray-400">
            Comparte esta landing page en LinkedIn para ayudarme a encontrar los socios ideales
          </p>
          <div className="flex justify-center gap-4 mt-4">
            <button
              onClick={() => {
                if (navigator.share) {
                  navigator.share({
                    title: 'App de Entrenamiento con IA - Buscando Socios',
                    text: 'Una aplicación revolucionaria de salud personalizada con IA. ¡Únete!',
                    url: window.location.href,
                  });
                } else {
                  navigator.clipboard.writeText(window.location.href);
                  alert('¡Enlace copiado al portapapeles!');
                }
              }}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors duration-300"
            >
              Compartir
            </button>
          </div>
        </div>
      </div>
    </section>
  );
}
